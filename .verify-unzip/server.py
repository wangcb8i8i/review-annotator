...
Usage:
    python3 server.py [--port 23456] [--plans-dir /path/to/plans]
"""
...
def guess_static_content_type(path: Path) -> str:
...
class PlanReviewerHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path.startswith("/res/"):
            rel_path = path[len("/res/"):]
            static_path = (RES_DIR / rel_path).resolve()

            try:
                static_path.relative_to(RES_DIR.resolve())
            except ValueError:
                self.send_404()
                return
            ...
            if static_path.exists() and static_path.is_file():
                ...
                self.send_response(200)
                self.send_header("Content-Type", guess_static_content_type(static_path))
                self.send_header("Content-Length", str(len(body)))
...
def main():
    ...
    if args.plans_dir:
        ...
        COMMENTS_DIR = PLANS_DIR.with_name(f"{PLANS_DIR.name}-reviews")
